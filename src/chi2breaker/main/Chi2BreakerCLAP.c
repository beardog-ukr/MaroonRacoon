...
int getChi2BreakerClapError(const int errorCode, char* line, const int lineLim) {
  ...
  switch (errorCode) {
    ...
    break;
    case EC_MISSED_ARGUMENT_FI:
      snprintf(line, lineLim, "Missed argument for -f (--fi) option\n");
    break;
    ...
    break;
    case EC_MISSED_FI:
      snprintf(line, lineLim, "Missed frequency info file name (-f or --fi option)\n");
    break;
...
void printChi2BreakerClapError(const int errCode) {
  const int ebs = 1024;
  char errbuf[ebs];
  if ( getChi2BreakerClapError(errCode, errbuf, ebs) ==0) {
    fprintf(stderr, "%s\n", errbuf);
  }
  else {
    fprintf(stderr, "Unknown internal error (Chi2breaker, %u)\n", errCode);
  }
}
...
int processCommandLine(int argc, char *argv[], Chi2BreakerParameters* parameters) {
  ...
  int opt;
  int option_index = 0;
  while ((opt = getopt_long(argc, argv, ":i:o:a:k:f:h", long_options, &option_index )) != -1) {
    switch (opt) {
      case 'i':
      ...
      case 'k':
        ...
        parameters->keyLength = strtol(optarg, 0, 10);
      break;
      case 'f':
        if (looksLikeOption(optarg)) {
          return EC_MISSED_ARGUMENT_FI;
        }
        parameters->freqFilename = optarg;
      break;
