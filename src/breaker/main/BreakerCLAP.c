...
int getBreakerClapError(const int errorCode, char* line, const int lineLim) {
  ...
  switch (errorCode) {
    ...
    break;
    case EC_MISSED_FI:
      snprintf(line, lineLim, "Missed letters frequency info file (-f or --fi option)\n");
    break;
...
void printBreakerClapError(const int errCode) {
  const int ebs = 1024;
  char errbuf[ebs];
  if ( getBreakerClapError(errCode, errbuf, ebs) ==0) {
    fprintf(stderr, "%s\n", errbuf);
  }
  else {
    fprintf(stderr, "Unknown internal error (breaker, %u)\n", errCode);
  }
}
...
int processCommandLine(int argc, char *argv[], BreakerParameters* parameters) {
  ...
  int opt;
  int option_index = 0;
  while ((opt = getopt_long(argc, argv, ":i:o:a:f:h", long_options, &option_index )) != -1) {
    switch (opt) {
      case 'i':
